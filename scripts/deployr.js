...
const main = async () => {
    const nftContractFactory = await hre.ethers.getContractFactory('BudgetNFT');
    ...
    const cloneContractFactory = await hre.ethers.getContractFactory('Deployer');
    const cloneContract = await cloneContractFactory.deploy();
    await cloneContract.deployed();
    console.log("Clone Contract deployed to:", cloneContract.address);
